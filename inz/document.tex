...
\begin{itemize}
  ...
  kolejności bajtów;
  \item Interpretowany - program nie jest kompilowany tylko przechowywnany w
...
\begin{itemize}
  \item JavaScript - jest to skryptowy jezyk programowania stosowany głównie do
  ...
  treśli strony. Skłądnia języka JavaScript jest zbudowana na podstawie języka
  C. Język umożliwia dynamiczne typowanie oraz jest obiektowy;
  \item HTML - jest to jeżyk znaczników służacy do tworzenia stron
  internetowych. Język skłąda się z tagów umieszczonych w \texttt{<tag>},
  ...
  stylów CSS, który służy do definiowania wyglądu i uładu tekstu i innych na
  stronie;
  \item XML - jest to język znaczników przeznaczony do reprezenowania danych w
  ...
  aplikacja uruchomipna na komputerze serwerem. HTTP jest protokołem warstwy
  apliacji, która zapewnia komunikację.
  \item \ldots
\end{itemize}
\section{db - MySQL}
...
wielowątkowy dostęp z możliwośćią obsłużenia dużej ilości użytkowników. Serwer
...
Data Manipulation Language - dotyczące mainpulowania), DLL (ang. Data
...
Komendy DML:
\begin{itemize}
  \item \texttt{commit} - komituje wpisaną transakcję, wrpowadzona zmiana jest
  pernamenta,
...
uruchomiony na Adroidzie jest uruchamiany na mszynie wirtualnej i jest
niezależny od pozostałych aplikacji. Najnowższą wersją systemu jest Android
Lollipop 5.0.
...
System Android składa się z czterech podsstawoych elementów. 
...
\begin{itemize}
  ...
  \item Services - jest to skłądnik aplikacji, który działa w tle, np. w
  urządzeniu odtwrzana jest muzyka w tle, a użytkownik korzysta z innej
  ...
  implemntuje się używając podklasy \texttt{ContentProvider};
  \item Broadcast receivers - jest komponenentem systemu, który jest
  odpowiedzialny za rozgłasznie informacji po systemie, np. infromacja o
  niskim stnie baterii. Taką właściwość implementuje się korzystając z podklasy
